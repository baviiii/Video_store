...
def phone_format(phone)
  ...
  if parsed.country == Phonelib.default_country.presence
  ...
  def sort_column_link(collection, key, text=nil, &block)
    ...
    if current_sort == key.to_s
      if new_params[:reverse_order].to_s == 'true' || current_reverse
        new_params[:reverse_order] = false
      else
        new_params[:reverse_order] = true
      end
  ...
  def format_detail_value(value, input)
    if value.present?
      ...
      formatted_value = %w[day_of_month day_of_next_month business_days calendar_days].
        include?(value) ? nil : value.to_date rescue nil
  ...
  def select_list(list, current_selection, type, text_method = 'to_s', value_method = 'id')
    if type == 'enum'
      ...
      current_selection.to_a.each do |selection|
        unless list.map(&:to_s).include?(selection)
          select =
            select.unshift(
              [
                selection.titleize,
      ...
      current_selection.to_a.each do |selection|
        unless list.map(&:id).include?(selection.id)
          select =
            select.unshift(
  ...
  def template_token(models)
    ...
    models.each do |model|
      liquid = (model.is_a?(Class) ? model : model.to_s.constantize).new.to_liquid rescue nil
      ...
      tokens.each do |token|
        ...
        liquid.send( "#{token}_description".to_sym )}<br/>" if
          liquid.respond_to?("#{token}_description".to_sym, true)
