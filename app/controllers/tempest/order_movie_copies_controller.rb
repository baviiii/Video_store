class Tempest::OrderMovieCopiesController < AdminController
  ...
  def index
    index_setup
    @title = 'Order Movie Copies' unless @title
  
    @order_movie_copies = apply_scopes(@order_movie_copies.includes(:movie_copy, :order))
      ...
      if @order_movie_copy.update(order_movie_copy_params)
        format.html do
          if request.xhr?
           ...
           redirect_to edit_admin_order_movie_copy_path(@order_movie_copy), notice: 'Order Movie Copy was successfully updated.'
