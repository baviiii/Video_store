class Tempest::AdminController < ApplicationController
  ...
  skip_authorization_check only: %i[index show edit destroy]
  ...
  before_action except: :destroy do
    if current_ability.respond_to?(:mvi_permitted_attributes)
      ...
      act = @override_action || action_name.to_sym
  ...
  before_action except: %i[create destroy], unless: :devise_controller? do
  ...
  after_action except: %i[create destroy], unless: :devise_controller? do
  ...
  def fetch_store(store_name, extra_ignored_params = nil)
    if params[:commit] == 'Clear'
      ...
      saved_params = params.dup.to_unsafe_h.delete_if { |k, v| ignored_params.include? k }
  ...
  def set_controller_crumb
    if controller_name != 'home' && respond_to?("admin_#{controller_name}_index_path")
      add_breadcrumb controller_label, send("admin_#{controller_name}_index_path")
    elsif controller_name != 'home' && respond_to?("admin_#{controller_name}_path")
