class UsersDashboard::MoviesController < ApplicationController
  ...
  # users_dashboard/movies_controller.rb
    def index
        ...
        if params[:by_genre].present?
          @movies = @movies.by_genre(params[:by_genre])
        end
      ...
      respond_with(@movies) do |format|
        format.html do
          ...
          if params[:commit] == 'Clear'
            redirect_to polymorphic_path([:users_dashboard,:movies])
          elsif request.xhr?
